...
class DocumentoTest extends BaseTest {
	...
	@Test
	void metricaTexto() {
		assertEquals(0.803921568627451, this.documentoController.recuperaMetricaTextoUtil(TEXTO1_ID));
	}
	
	@Test
	void concatecaDocumentos() {
	...
	void sumarizaTest() {
			String sum = "";
			for (String s : this.documentoController.sumariza(JAVA_ID)) {
				sum += s + " ";
			}
			assertEquals("package Optional HistoricoBusca DocumentoDTO ResultadoSimilaridade ", sum);
